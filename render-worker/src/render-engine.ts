...
import { EnvironmentMode, getConfig } from './config/environment';
...
export const renderEngine = {
    async renderDocumentary(
        ...
        if (onProgress) await onProgress(0, `Starting ${isVertical ? 'Vertical (9:16)' : `${targetRes} Horizontal (16:9)`} render...`);
        ...
        try {
            if (onProgress) await onProgress(5, `Downloading ${scenes.length} assets...`);
            ...
            // 7. Upload to Storage
            if (onProgress) await onProgress(98, '🚀 UPLOADING TO HI-RES CLOUD...');
    ...
    async preProcessImage(projectId: string, src: string, dest: string, aspectRatio: '16:9' | '9:16' = '16:9', resolution: string = '4k') {
        const isVertical = aspectRatio === '9:16';
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            let cmdLine = '';
            command.on('start', (cmd: string) => { cmdLine = cmd; console.log(`[FFmpeg] Scale: ${cmd}`); })
                .outputOptions([
    ...
    async createSteadySegment(projectId: string, imgPath: string, duration: number, outPath: string) {
        return new Promise<void>((resolve, reject) => {
            ...
            let cmdLine = '';
            command.on('start', (cmd: string) => { cmdLine = cmd; console.log(`[FFmpeg] Steady Segment: ${cmd}`); })
                .loop(duration)
    ...
    async createVideoSegment(projectId: string, vidPath: string, duration: number, outPath: string, aspectRatio: '16:9' | '9:16' = '16:9', resolution: string = '4k') {
        const isVertical = aspectRatio === '9:16';
        const RESOLUTIONS: Record<string, { w: number, h: number }> = {
            '4k': { w: 3840, h: 2160 },
            '1080p': { w: 1920, h: 1080 },
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            let cmdLine = '';
            command.on('start', (cmd: string) => { cmdLine = cmd; console.log(`[FFmpeg] Video Segment (${resolution}): ${cmd}`); })
                .outputOptions([
                    '-t', duration.toString(),
    ...
    async createTransitionSegment(projectId: string, imgAPath: string, imgBPath: string, type: string, duration: number, outPath: string) {
        return new Promise<void>((resolve, reject) => {
            ...
            command.input(imgAPath).inputOptions(['-loop 1', `-t ${duration}`])
                ...
                .on('start', (cmdLine: string) => console.log(`[FFmpeg] Transition: ${cmdLine}`))
    ...
    async generateCineVideo(
        ...
        try {
                ...
                for (let j = 0; j < batchSize && (i + j) < primaryScenes.length; j++) {
                    ...
                    batchPromises.push((async () => {
                        ...
                        if (steadyDuration > 0.05) {
                            const steadyPath = path.join(tempDir, `primary-steady-${sceneIdxInPrimary}.mp4`);
                        ...
                        if (transitionDuration > 0) {
                            const transPath = path.join(tempDir, `primary-trans-${sceneIdxInPrimary}.mp4`);
            ...
            if (onProgress) await onProgress(70, 'Stitching primary track...');
            ...
            if (onProgress) await onProgress(95, 'Finalizing mix with overlays...');
    ...
    async muxFinalAudio(
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            command.outputOptions(outputOptions)
                .on('start', (cmd: string) => { cmdLine = cmd; console.log(`[FFmpeg] Mux: ${cmd}`); })
                .on('end', () => {
                    clearTimeout(timeout);
                    untrackCommand(projectId, command);
                    resolve();
