    ...
    try {
        ...
        renderEngine.renderDocumentary(
            ...
            async (progress, message) => {
                // Periodically update progress in Firestore for this clip
                // We use a separate update call to avoid race conditions or bloat
                ...
                if (projectNow && projectNow.shorts) {
                    ...
                    if (idx !== -1) {
                        projectNow.shorts[idx].renderProgress = progress;
        ...
        return NextResponse.json({
            success: true,
            message: 'Short rendering started in background',
            clipId
        });
