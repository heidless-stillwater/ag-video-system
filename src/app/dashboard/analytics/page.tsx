...
export default function AnalyticsDashboard() {
    ...
    const byService = logs.reduce((acc, log) => {
        acc[log.service] = (acc[log.service] || 0) + (log.estimatedCost || 0);
        return acc;
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <KpiCard title="Total Estimated Cost" value={`$${totalCost.toFixed(4)}`} icon="💰" />
                <KpiCard title="API Calls Traced" value={logs.length.toString()} icon="📡" />
                <KpiCard title="Vertex AI Cost" value={`$${(byService['vertex-ai'] || 0).toFixed(4)}`} icon="🧠" />
                <KpiCard title="Rendering Cost" value={`$${(byService['render'] || 0).toFixed(4)}`} icon="🎬" />
            </div>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading ? (
                                ...
                                <tr key={log.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4 text-right font-mono text-green-400/90">${log.estimatedCost.toFixed(5)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
