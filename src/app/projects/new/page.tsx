...
function NewProjectContent() {
    ...
    useEffect(() => {
        const personaFromUrl = searchParams.get('personaId');
        if (personaFromUrl) {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            try {
                ...
                // Fetch Personas
                const personasRes = await authFetch('/api/research/personas');
                const personasData = await personasRes.json();
                if (personasData.success) {
                    setPersonas(personasData.personas);
                    if (personasData.personas.length > 0) {
                        const saved = localStorage.getItem('last_research_persona');
                        if (!saved) setSelectedPersona(personasData.personas[0].id);
                    }
                }
            } catch (err: any) {
    ...
    const handleCreateProject = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const projectData = {
                ...
                title,
                description,
                broadCategory: topic.broadCategory,
                topicId: topic.id,
                ...
                estimatedCredits: 5,
                creditsDeducted: 0,
                visualStyle: 'studio-ghibli' as any, // Default will be selectable later
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <form onSubmit={handleCreateProject} className="space-y-6">
                    <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl backdrop-blur-sm space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-4 uppercase tracking-widest text-[10px] font-black">Research Personas</label>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {personas.map(persona => (
                                    <button
                                        ...
                                        className={`p-5 rounded-3xl border-2 transition-all text-left flex flex-col gap-2 relative overflow-hidden group ${
                                            selectedPersona === persona.id
                                                ? 'bg-blue-600 border-blue-400 shadow-[0_0_30px_-5px_rgba(59,130,246,0.5)] scale-[1.02] z-10'
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={`font-black uppercase tracking-tight text-sm ${selectedPersona === persona.id ? 'text-white' : 'text-slate-200'}`}>
                                                {persona.name}
                                            </span>
                                        </div>
                                        <p className={`text-xs leading-relaxed line-clamp-2 ${selectedPersona === persona.id ? 'text-blue-50/80' : 'text-slate-400'}`}>
                                            {persona.description}
                                        </p>
                                    </button>
                                ))}
