...
export class ResourceGovernor {
     ...
     * @param options.bypassIfDelegated If true, allows operations even under load if they are likely to be offloaded.
     */
    public isSystemHealthy(options: { bypassIfDelegated?: boolean } = {}): { healthy: boolean; reason?: string } {
        const pressure = this.getSystemPressure();
        const freeMemGB = os.freemem() / (1024 * 1024 * 1024);

        // If we are delegating to the cloud, we can be much more relaxed about local CPU.
        // We still check RAM as it's needed for orchestration/manifest prep.
        const isDelegated = options.bypassIfDelegated && this.isRenderDelegationEnabled();

        if (pressure > 0.95 && !isDelegated) return { healthy: false, reason: 'CPU load is maxed out' };
        if (freeMemGB < 0.4) return { healthy: false, reason: 'Available memory is critically low (<400MB)' };

        return { healthy: true };
    }

    public isRenderDelegationEnabled(): boolean {
        return !!(process.env.CLOUD_RUN_RENDER_URL || process.env.NEXT_PUBLIC_CLOUD_RUN_RENDER_URL);
    }
}
