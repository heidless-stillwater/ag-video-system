...
import { Script, ScriptSection, VisualStyle, ViralCandidate, MediaAssetType } from '@/types';
import { videoEngine } from './video-engine';
import { mediaLibraryServerService } from './media-library-server';
import { PromptToolBridgeService } from './prompttool-bridge';
...
export async function generateDocumentaryScript(
    ...
    const personaNarrativeStyles: Record<string, string> = {
        'standard': 'Balanced, calm, and objective. A steady, informative guide through the facts.',
        'skeptic': 'Analytical and rigorous. You are a truth-seeker who questions common wisdom and focuses on verified reality, debunking myths with a calm, intellectual tone.',
        'poet': 'Whimsical and philosophical. You focus on the atmospheric beauty and deeper meaning of the subject, using evocative and lyrical language.',
        'historian': 'Deeply rooted in the past. You emphasize the flow of time, the weight of epochs, and how ancient foundations led to the present day.',
        'scientist': 'Scientific and precise. You find wonder in numbers, measurements, and technical mechanisms, providing an objective, breakdown-oriented perspective.'
    };

    const narrativeStyle = personaNarrativeStyles[persona] || personaNarrativeStyles['standard'];
    const quoteContext = quotes.length > 0
        ? `INCORPORATE THE FOLLOWING QUOTES THOSE CORRESPOND TO YOUR PERSONA:\n${quotes.map(q => `"${q.text}" - ${q.author}`).join('\n')}`
        : '';

    const prompt = `
        You are a world-class scriptwriter for a "Sleep Documentary" channel called Stillwater. 
        ...
        - VOCABULARY DEPTH: ${vocabularyComplexity.toUpperCase()}
        - SLEEP INDUCTION LEVEL: ${sleepInducingLevel.toUpperCase()}
        - AMBIENT CONTEXT/ATMOSPHERE: ${atmosphere}
        ...
        1. Low-Arousal Content: Even while maintaining your ${persona.toUpperCase()} persona, keep the delivery soothing. No sudden shifts or exciting hooks.
        2. Adjusted Vocabulary: Use ${vocabularyComplexity} language that resonates with ${targetAudience}. 
        3. Pacing: Write for a slow, rhythmic delivery at ${targetPacing} words per minute.
        4. Atmosphere Injection: Weave the ambient context ("${atmosphere}") into the metaphors and intro/outro.
        5. Deepening Sleep: As the script progresses into later sections, make the language even more repetitive and rhythmic to assist the ${sleepInducingLevel} sleep induction goals.
        6. Persona Integration: If you are a poet, use metaphors. If a skeptic, dismantle myths gently. If a data scientist, find the magic in the scale of numbers.
        7. Length Strategy: Target exactly ${Math.round(targetDuration * targetPacing)} words total.

        STRUCTURE:
        - Introduction: Setting the scene through the ${persona} lens and the ${atmosphere} atmosphere.
        - 3-4 Deep-Dive Sections: Exploring the research facts.
        - Conclusion: A gentle, lingering sequence that leads the listener into a peaceful state.

        Format the output as a JSON object with a "sections" array:
        {
          "sections": [
            { "title": "Section Title", "content": "The full narration text...", "wordCount": 123 }
          ]
        }
    `;
...
export async function extractFacts(content: string, persona: string = 'standard', envMode?: EnvironmentMode): Promise<{ facts: string[], quotes: { author: string, text: string }[] }> {
    const personaPrompts: Record<string, string> = {
        'standard': 'Extract interesting, factual, and calming points.',
        'skeptic': 'Extract facts while critically evaluating common myths or misconceptions. Be precise and detail-oriented.',
        'poet': 'Extract facts that are particularly beautiful, atmospheric, or philosophical in nature.',
        'historian': 'Extract facts that focus on the timeline, cultural origins, and evolutionary context of the topic.',
        'data-scientist': 'Extract facts that are heavy on statistics, numbers, scientific measurements, and objective data.'
    };

    const personaPrompt = personaPrompts[persona] || personaPrompts['standard'];

    const prompt = `
        You are a research assistant for a high-quality "sleep documentary" YouTube channel.
        Your persona for this research is: ${persona.toUpperCase()}.
        ${personaPrompt}

        Extract 10-15 points from the following content.
        Focus on historical details, natural phenomena, or scientific wonders.
        Also, extract any notable quotes from historical figures or experts that would fit a calming documentary.
        ...
        {
          "facts": ["Fact 1", "Fact 2", ...],
          "quotes": [{"author": "Author Name", "text": "Quote text"}, ...]
        }
    ...
    try {
        ...
        console.error('[AI Service] Failed to parse extracted facts JSON, returning fallback');
        // Simple fallback parsing if JSON fails
        const facts = response.split('\n')
            .map(line => line.replace(/^[-*•]\s+/, '').trim())
            .filter(line => line.length > 0 && !line.toLowerCase().includes('here are') && !line.includes('{'));
        return { facts, quotes: [] };
    }
}
...
export async function generateVisualCues(
    ...
    imageCount?: number,
    optimization: 'thematic' | 'even' = 'thematic'
): Promise<{
    timestamp: number;
    type: 'image' | 'video';
    description: string;
    ...
    // Determine cue count instructions
    let cueCountPrompt = `generate ${imageCount || '3-5'} visual 'cues'.`;
    if (imageCount) {
        cueCountPrompt = `generate exactly ${imageCount} high-impact cinematic visual 'cues'.`;
    } else if (config.ai.limitAI) {
        cueCountPrompt = "generate exactly 1 high-impact cinematic visual 'cue'.";
    }

    const optimizationPrompt = optimization === 'even' 
        ? "Space the cues mathematically evenly across the estimated duration of the content."
        : "Identify key narrative shifts, emotional beats, or descriptive changes in the content to place cues at the most thematic moments.";
    ...
    const prompt = `
        ...
        ATMOSPHERIC CUES/PROMPT GUIDE: ${atmosphericCues || 'Follow the visual style.'}
        CONTENT: ${sectionContent}

        STRATEGY: ${optimizationPrompt}
        ...
        5. Pacing: Space the cues according to the STRATEGY provided above.
...
export async function generateImage(
    ...
    visualConfig: {
        ...
        userId?: string; // Unified user ID for auto-logging
        projectId?: string;
        sectionId?: string;
    } = {},
    retryCount = 0
    ...
    const engine = visualConfig.engine || 'nanobanana-2';

    // Image Resources API or Mock Mode -> Use Unsplash
    if (config.ai.model === 'mock' || engine === 'stock-api') {
        ...
        if (visualStyle === 'cyberpunk' || visualStyle === 'techno') index = 6;
        if (visualStyle === 'oil-painting') index = 5;
        if (visualStyle === 'watercolor' || visualStyle === 'dreamy') index = 7;

        const randomSig = Math.floor(Math.random() * 1000000) + Date.now();
        const baseImageUrl = placeholders[index];
        
        // Image Resources API gets higher quality parameters
        const quality = engine === 'stock-api' ? 'q=95&w=1600' : 'q=80&w=1000';
        
        return `${baseImageUrl}?auto=format&fit=crop&${quality}&sig=${randomSig}`;
    }

    if (engine === 'manual-upload') {
        // Return a placeholder that indicates manual upload is needed
        // The UI will handle displaying this as an 'upload required' state
        return '/placeholders/waiting-for-upload.jpg';
    }
    ...
    try {
        ...
        const STYLE_METADATA: Record<VisualStyle, string> = {
            ...
            'noir': 'film noir aesthetic, high contrast black and white, dramatic hard shadows, moody atmosphere, 1940s cinema look',
            'minimalist': 'clean minimalist composition, lots of negative space, simple geometric forms, subtle textures, peaceful clarity',
            'dreamy': 'dreamy ethereal atmosphere, soft focus, light leaks, pastel glows, surreal and peaceful',
            'techno': 'techno-organic fusion, neon bioluminescence, intricate mechanical details, futuristic dark aesthetic'
        };

        const stylePrompt = STYLE_METADATA[visualStyle] || STYLE_METADATA['cinematic'];
        const palettePrompt = visualConfig.palette ? `Color Palette Influence: ${visualConfig.palette}. ` : '';
        const atmosphericPrompt = visualConfig.atmosphericCues ? `Atmospheric details: ${visualConfig.atmosphericCues}. ` : '';
        const motionPrompt = visualConfig.motionIntensity ? `Capture the feeling of ${visualConfig.motionIntensity} motion. ` : '';
        const proEnhancement = engine === 'nanobanana-pro' ? 'Award-winning composition, incredible textures, masterwork lighting, sharpest focus, 16k raw detail. ' : '';
        
        const enhancedPrompt = `${stylePrompt}: ${proEnhancement}${palettePrompt}${atmosphericPrompt}${motionPrompt}${prompt}`;

        const modelId = engine === 'nanobanana-pro' ? 'imagen-3.0-generate-001' : 'imagen-3.0-fast-generate-001';
        ...
        if (error.response?.status === 429 && retryCount < 3) {
            ...
            return generateImage(prompt, envMode, visualStyle, visualConfig, retryCount + 1);
        }
