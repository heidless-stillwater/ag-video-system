...
interface UseMediaActionsReturn {
    ...
    handleGenerateMedia: () => Promise<void>;
...
export const useMediaActions = ({
    ...
    const handleCancelMedia = async () => {
        if (!project) return;
        try {
            await fetch(`/api/projects/${project.id}/media/cancel`, { method: 'POST' });
        } catch (err) {
    ...
    const handleKillRender = async () => {
        if (!project) return;
        try {
            await fetch(`/api/projects/${project.id}/render/kill`, { method: 'POST' });
