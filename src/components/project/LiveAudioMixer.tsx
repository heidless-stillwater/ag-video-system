    ...
    return (
        <div className="mt-6 p-6 bg-gradient-to-br from-purple-900/20 to-slate-900/40 border border-purple-500/20 rounded-2xl space-y-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => {
                            ...
                            onLiveVolumeChange?.('narrationVolume', initialNarrationVolume);
                            
                            setMusicVolume(initialMusicVolume);
                            onLiveVolumeChange?.('backgroundMusicVolume', initialMusicVolume);
                            
                            setAmbianceVolume(initialAmbianceVolume);
                            onLiveVolumeChange?.('ambianceVolume', initialAmbianceVolume);
                            
                            setSfxVolume(initialSfxVolume);
                            onLiveVolumeChange?.('globalSfxVolume', initialSfxVolume);
                            
                            setVideoVolume(initialVideoVolume);
                            onLiveVolumeChange?.('videoVolume', initialVideoVolume);
                            
                            setAutoDucking(initialAutoDucking);
                            onLiveVolumeChange?.('autoDucking', initialAutoDucking);
                        }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">🎬 Video Sound</label>
                        <span className="text-xs font-mono text-purple-400">{Math.round(videoVolume * 100)}%</span>
                    </div>
                    <input
                        type="range"
                        min="0"
                        ...
                        onChange={(e) => {
                            const val = parseFloat(e.target.value);
                            setVideoVolume(val);
                            onLiveVolumeChange?.('videoVolume', val);
                        }}
                        className="w-full accent-purple-500"
                    />
                </div>
