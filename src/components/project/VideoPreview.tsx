...
import { useDuckingEffect, useVolumeSync } from '@/lib/hooks/useAudioEffects';
...
interface VideoPreviewProps {
    ...
    autoDucking?: boolean;
    onLiveVolumeChange?: (key: 'narrationVolume' | 'backgroundMusicVolume' | 'ambianceVolume' | 'globalSfxVolume' | 'videoVolume' | 'autoDucking', value: number | boolean) => void;
    initialTime?: number;
}
...
export const VideoPreview: React.FC<VideoPreviewProps> = ({
    ...
    const initialSceneIndex = useMemo(() => {
        const idx = scenes.findIndex(s => initialTime >= s.startTime && initialTime < (s.startTime + s.duration));
        return idx === -1 ? 0 : idx;
    }, [scenes, initialTime]);
    ...
    const [absoluteCurrentTime, setAbsoluteCurrentTime] = useState(initialTime);
    ...
    const [tempVideoVol, setTempVideoVol] = useState(videoVolume ?? 0.8);
    ...
    useEffect(() => {
        if (currentScene) {
            console.log(`[VideoPreview] Scene ${currentSceneIndex}:`, {
                id: currentScene.id,
                img: currentScene.imageUrl?.substring(0, 30),
                vid: currentScene.videoUrl?.substring(0, 30),
                bufferActive: buffer.active,
                bufferImgA: buffer.imgA?.substring(0, 30),
                bufferVidA: buffer.vidA?.substring(0, 30)
            });
            setDebugInfo(`Scene ${currentSceneIndex} | ${currentScene.videoUrl ? 'VIDEO' : 'IMAGE'}`);
        }
    }, [currentScene?.id, buffer.active, currentSceneIndex]);
    ...
    // Handle global volume for videos
    useEffect(() => {
        if (videoRefA.current) videoRefA.current.volume = tempVideoVol;
        if (videoRefB.current) videoRefB.current.volume = tempVideoVol;
    }, [tempVideoVol]);

    // Handle global muting across all internal audio/video elements
    useEffect(() => {
        if (audioRef.current) audioRef.current.muted = isGlobalMuted;
        if (musicRef.current) musicRef.current.muted = isGlobalMuted;
        if (ambianceRef.current) ambianceRef.current.muted = isGlobalMuted;
        if (sceneSfxRef.current) sceneSfxRef.current.muted = isGlobalMuted;
        if (sfxRef.current) sfxRef.current.muted = isGlobalMuted;
        // Videos are already bound via React props: muted={isGlobalMuted}
    }, [isGlobalMuted]);
    ...
    useEffect(() => {
        ...
        // Trigger a swap if the scene ID changes OR if the transition properties change 
        // (to ensure "Sync" updates are visually reflected immediately).
        const sceneKey = `${currentScene.id}-${currentScene.transitionType || 'fade'}-${currentScene.transitionDuration || 1000}`;
        
        if (sceneKey !== lastSceneIdRef.current) {
            setAudioError(null);
            const nextImg = currentScene.imageUrl;
            const nextVid = currentScene.videoUrl;
            
            // Always cycle isImageReady to false to ensure the CSS transition animations 
            // from getTransitionStyles are triggered when the buffer flips.
            ...
            lastSceneIdRef.current = sceneKey;
        }
    ...
    // Video Sync Logic
    useEffect(() => {
        const activeVideo = buffer.active === 'A' ? videoRefA.current : videoRefB.current;
        const inactiveVideo = buffer.active === 'A' ? videoRefB.current : videoRefA.current;

        // Pause the inactive buffer
        if (inactiveVideo) inactiveVideo.pause();

        if (activeVideo && currentScene?.videoUrl) {
            if (isPlaying) {
                const relTime = absoluteCurrentTime - currentScene.startTime;
                // Avoid constant setting to prevent flicker, but keep in sync
                if (Math.abs(activeVideo.currentTime - relTime) > 0.3) {
    ...
    }, [isPlaying, absoluteCurrentTime, buffer.active, currentScene?.id]);

    const handleImageLoad = useCallback(() => {
        console.log(`[VideoPreview] Media loaded. Ready for transition.`);
        requestAnimationFrame(() => {
            setTimeout(() => {
                setIsImageReady(true);
                setBuffer(prev => ({ ...prev, zoomA: prev.active === 'A', zoomB: prev.active === 'B' }));
            }, 50); // Reduced delay for snappier transitions
        });
    }, []);
    ...
    const togglePlayback = () => {
        if (isPlaying) {
            ...
            // Explicitly pause everything immediately
            if (audioRef.current) audioRef.current.pause();
            if (musicRef.current) musicRef.current.pause();
            if (ambianceRef.current) ambianceRef.current.pause();
            if (sceneSfxRef.current) sceneSfxRef.current.pause();
    ...
    useVolumeSync({
        channel: "Narration",
        refEl: audioRef,
        volume: tempNarrationVol,
        trigger: currentScene?.sectionId
    });
    ...
    const getTransitionStyles = (isCurrent: boolean, type: string, duration: number) => {
        ...
        const styles: React.CSSProperties = {
            transition: `opacity ${time} cubic-bezier(0.4, 0, 0.2, 1), transform ${time} cubic-bezier(0.4, 0, 0.2, 1), filter ${time} cubic-bezier(0.4, 0, 0.2, 1)`,
        ...
        <div className={isInline
            ...
            <div className={isInline
                ...
                >
                    ...
                    <div {...getTransitionStyles(
                        ...
                        {buffer.vidA ? (
                            <div className="w-full h-full relative">
                                <video
                                    key={`vid-a-${buffer.vidA}-${lastSceneIdRef.current}`}
                                    ref={videoRefA}
                                    src={buffer.vidA}
                                    className="w-full h-full object-cover"
                                    muted={isGlobalMuted}
                                    playsInline
                                    onLoadedData={buffer.active === 'A' ? handleImageLoad : undefined}
                                    onError={() => {
                                        console.warn('[VideoPreview] Failed to load VidA:', buffer.vidA);
                                        if (buffer.active === 'A') handleImageLoad();
                                    }}
                                    loop
                                />
                                {/* Image fallback overlay while video loads */}
                                {buffer.active === 'A' && !isImageReady && buffer.imgA && (
                                    <img key={`vid-a-fb-${buffer.imgA}`} src={buffer.imgA} className="absolute inset-0 w-full h-full object-cover" alt="" />
                                )}
                            </div>
                        ) : buffer.imgA && (
                            <img
                                key={`img-a-${buffer.imgA}-${lastSceneIdRef.current}`}
                                src={buffer.imgA}
                    ...
                    <div {...getTransitionStyles(
                        ...
                        {buffer.vidB ? (
                            <div className="w-full h-full relative">
                                <video
                                    key={`vid-b-${buffer.vidB}-${lastSceneIdRef.current}`}
                                    ref={videoRefB}
                                    src={buffer.vidB}
                                    className="w-full h-full object-cover"
                                    muted={isGlobalMuted}
                                    playsInline
                                    onLoadedData={buffer.active === 'B' ? handleImageLoad : undefined}
                                    onError={() => {
                                        console.warn('[VideoPreview] Failed to load VidB:', buffer.vidB);
                                        if (buffer.active === 'B') handleImageLoad();
                                    }}
                                    loop
                                />
                                {buffer.active === 'B' && !isImageReady && buffer.imgB && (
                                    <img key={`vid-b-fb-${buffer.imgB}`} src={buffer.imgB} className="absolute inset-0 w-full h-full object-cover" alt="" />
                                )}
                            </div>
                        ) : buffer.imgB && (
                            <img
                                key={`img-b-${buffer.imgB}-${lastSceneIdRef.current}`}
                                src={buffer.imgB}
                ...
                <div className={`w-full ${isInline ? '' : 'max-w-5xl'} space-y-8 bg-white/[0.02] backdrop-blur-xl p-8 rounded-[32px] border border-white/5 shadow-2xl flex-shrink-0`}>
                    ...
                    {showOverlays && (
                        <div className="flex flex-col items-center gap-2 border-t border-white/5 pt-6 mt-4">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
                                Technical Frame Data
                            </p>
                            <div className="flex items-center gap-4 opacity-40 hover:opacity-100 transition-opacity">
                                <p className="text-[9px] text-blue-400 font-mono tracking-widest">{currentScene?.sectionTitle}</p>
                                <span className="w-1 h-1 rounded-full bg-white/20"></span>
                                <p className="text-[9px] text-slate-300 font-mono tracking-widest italic">{currentScene?.cueDescription}</p>
                                <span className="w-1 h-1 rounded-full bg-white/20"></span>
                                <p className="text-[9px] text-amber-500 font-mono tracking-widest">{currentScene?.transitionType} ({currentScene?.transitionDuration}ms)</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg transition-all ${isGlobalMuted ? 'bg-red-600/20 text-red-400 border border-red-500/30' : 'bg-slate-800 text-slate-500 border border-slate-700 hover:text-white'}`}
                    ...
                    {showMixer && (
                        <LiveAudioMixer
                            ...
                            setNarrationVolume={setTempNarrationVol}
                            musicVolume={tempMusicVol}
                            setMusicVolume={setTempMusicVol}
                            ambianceVolume={tempAmbianceVol}
                            setAmbianceVolume={setTempAmbianceVol}
                            sfxVolume={tempSfxVol}
                            setSfxVolume={setTempSfxVol}
                            autoDucking={tempAutoDucking}
                            setAutoDucking={setTempAutoDucking}
                            videoVolume={tempVideoVol}
                            setVideoVolume={setTempVideoVol}
                            initialNarrationVolume={1.0}
                            initialMusicVolume={0.2}
                            initialAmbianceVolume={0.1}
                            initialSfxVolume={0.4}
                            initialVideoVolume={0.8}
                            initialAutoDucking={true}
                            onLiveVolumeChange={onLiveVolumeChange}
                            onSave={onSaveAudioSettings}
                            isSaving={isSavingAudio}
                        />
                    )}
            ...
            {currentScene?.audioUrl && (
                <audio
                    ...
                    muted={isGlobalMuted}
                    onPlay={() => {
                        console.log(`[AUDIO:NARRATOR] ▶️ Playback started: ${currentScene?.audioUrl?.slice(-20) || 'unknown'}`);
                        setIsNarratorActuallyPlaying(true);
                        if (audioRef.current) audioRef.current.volume = tempNarrationVol;
                    }}
                    onPause={() => {
                        console.log(`[AUDIO:NARRATOR] ⏸️ Playback paused`);
                        setIsNarratorActuallyPlaying(false);
                    }}
                    onEnded={() => {
                        console.log(`[AUDIO:NARRATOR] 🏁 Section ended`);
                        setIsNarratorActuallyPlaying(false);
                        handleAudioEnded();
                    }}
                    onError={() => {
                        console.error(`[AUDIO:NARRATOR] ❌ Error loading: ${currentScene.audioUrl}`);
            ...
            {backgroundMusicUrl && (
                <audio
                    ...
                    loop
                    muted={isGlobalMuted}
                    onPlay={() => console.log(`[AUDIO:MUSIC] ▶️ Playback started: ${backgroundMusicUrl?.slice(-20) || 'unknown'}`)}
                    onPause={() => console.log(`[AUDIO:MUSIC] ⏸️ Playback paused`)}
                    onError={(e) => {
            ...
            <audio
                ...
                muted={isGlobalMuted}
                onPlay={() => console.log(`[AUDIO:SFX-CHIME] ▶️ Playback started`)}
                onPause={() => console.log(`[AUDIO:SFX-CHIME] ⏸️ Playback paused`)}
                preload="auto"
            />
            ...
            <audio 
                ref={sceneSfxRef} 
                muted={isGlobalMuted}
                onPlay={() => console.log(`[AUDIO:SCENE-SFX] ▶️ Playback started: ${sceneSfxRef.current?.src?.slice(-20)}`)}
                onPause={() => console.log(`[AUDIO:SCENE-SFX] ⏸️ Playback paused`)}
            />
            ...
            {ambianceUrl && (
                <audio
                    ...
                    loop
                    muted={isGlobalMuted}
                    onPlay={() => console.log(`[AUDIO:AMBIANCE] ▶️ Playback started: ${ambianceUrl?.slice(-20) || 'unknown'}`)}
                    onPause={() => console.log(`[AUDIO:AMBIANCE] ⏸️ Playback paused`)}
                    onError={(e) => {
                        const target = e.target as HTMLAudioElement;
                        const error = target.error;
